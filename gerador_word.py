...
class GeradorAnuenciaIncraWord:
    ...
    def _obter_dados_estruturados_com_ia(self, texto_memorial: str) -> Dict[str, Any]:
        ...
        Sua tarefa é analisar o texto de um memorial descritivo e estruturar as informações do IMÓVEL DE ORIGEM e de TODOS os confrontantes.
        ...
        Responda APENAS com o JSON estruturado abaixo:
        {{
            "imovel_origem": {{
        ...
        if nome_arquivo.lower().endswith(".pdf"):
            try:
                ...
                if pypdf:
                    ...
                    paginas_texto = [pagina.extract_text() for pagina in reader.pages if pagina.extract_text()]
                    texto_memorial = "\n".join(paginas_texto)
            except Exception as pdf_err:
                logger.error(f"Falha ao extrair PDF: {pdf_err}")
            ...
            try:
                ...
                logger.error(f"Falha ao extrair DOCX: {docx_err}")
        else:
    ...
    def gerar_documentos_pelo_memorial(
        ...
        for dados_confrontante in confrontantes:
            nome_confrontante = str(dados_confrontante.get("confrontante_proprietario", "Confrontante")).strip()
    ...
    def _formatar_decimal_estrito_2_casas(self, valor: Any) -> str:
        ...
        texto_limpo = re.sub(r'[^\d\.,]', '', str(valor).strip()).replace(",", ".")
        try:
            return f"{float(texto_limpo):.2f}"
        except ValueError:
            return texto_limpo

    def _definir_margens_celula(self, cell, top=30, bottom=30, left=50, right=50):
        """
        Zera ou reduz as margens internas da célula (padding) via manipulação de XML
        para que o texto ocupe toda a largura física da coluna sem quebrar linha à toa.
        """
    ...
    def _montar_documento_confrontante(
        ...
        for section in doc.sections:
            ...
            # Margens laterais otimizadas para dar o máximo de largura útil para a tabela
        ...
        p_corpo.paragraph_format.space_after = Pt(8)
        ...
        # 4. TABELA ULTRA AJUSTADA (Totalizando 10.4 polegadas de largura de página utilizável)
        # Encolhemos ao máximo as colunas de dados curtos para sobrar espaço para o confrontante
        larguras_colunas = [
            Inches(0.9),   # Código (G1D-M-XXXXX)
            Inches(1.05),  # Longitude
            Inches(1.05),  # Latitude
            Inches(0.7),   # Altitude (m)
            Inches(0.9),   # Código (Vante)
            Inches(0.6),   # Azimute
            Inches(0.6),   # Dist. (m)
            Inches(4.6)    # Confrontante (Aumentado para 4.6 polegadas!)
        ]
        ...
        for idx, text in enumerate(sub_headers):
            ...
            cell.text = text
            self._definir_margens_celula(cell, top=20, bottom=20, left=40, right=40)
        ...
        for v in vertices_dados:
            ...
            cells[1].text = self._formatar_coordenada_estrita(v.get("longitude", ""))
            cells[2].text = self._formatar_coordenada_estrita(v.get("latitude", ""))
            cells[3].text = self._formatar_decimal_estrito_2_casas(v.get("altitude", ""))
            cells[4].text = str(v.get("vante", ""))
            cells[5].text = self._formatar_azimute_estrito(v.get("azimute", ""))
            cells[6].text = self._formatar_decimal_estrito_2_casas(v.get("distancia", ""))
            cells[7].text = str(v.get("confrontacao_completa", ""))

        # Aplicar largura física exata, margens internas zeradas e tamanho de fonte estrito
        for r_idx, row in enumerate(tabela_vert.rows):
            for c_idx, cell in enumerate(row.cells):
                cell.width = larguras_colunas[c_idx]
                
                # Zera as margens da célula (padding) para expandir a área de texto útil
                self._definir_margens_celula(cell, top=20, bottom=20, left=40, right=40)
                ...
                if len(p.runs) > 0:
                    # Fonte 7.2 garante que os confrontantes caibam em uma única linha,
                    # mantendo a célula do código e as outras super compactas e perfeitamente alinhadas!
                    p.runs[0].font.size = Pt(7.2)
                    p.runs[0].font.name = 'Arial'

        # Espaço proporcional antes das Assinaturas
        doc.add_paragraph().paragraph_format.space_before = Pt(24)

        # 5. ASSINATURAS PARALELAS
        tabela_assinaturas = doc.add_table(rows=2, cols=2)
        tabela_assinaturas.autofit = False
        tabela_assinaturas.columns[0].width = Inches(5.2)
        tabela_assinaturas.columns[1].width = Inches(5.2)
